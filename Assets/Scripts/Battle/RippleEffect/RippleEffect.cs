...
public class RippleEffect : MonoBehaviour
{
	...
	public bool shouldStartRipple = false;
    ...
    void Update()
    {
		if (dropInterval > 0 && shouldStartRipple)
        {
			shouldStartRipple = false;
			Vector3 position = c.WorldToViewportPoint (ripplePosition);
			Emit (position);

            timer += Time.deltaTime;
            if (timer > dropInterval) timer -= dropInterval;
        }
